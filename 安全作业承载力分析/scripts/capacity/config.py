...
DEFAULT_INPUT_CONFIG: Dict[str, Any] = {
    ...
    "组织架构映射": {
        ...
        "表头行号": 0,
        "数据起始行号": 1,
        "说明": "是否启用=true 时把计划/班组的 公司/工区/班组 归一化到报告矩阵表的单位列(中心/公司)；输出结果 JSON 增加 matrix(日期×单位)。单位/工区/班组清单不再入库配置（v1.6 配置瘦身）：启用「优先读取源表组织架构」后按 xls「组织架构」sheet 内容动态解析归列（县公司简称∈报告单位列直接归县公司，市公司直属单位按行内「单位（工区）/工区简称」名归列）；sheet 未命中时用代码内置的归属关键词规则兜底。",
    },
}
